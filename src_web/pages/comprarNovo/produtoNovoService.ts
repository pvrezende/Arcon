// Serviço para buscar produtos novos
...
export interface ProdutoNovoFormatado {
  id: string;
  modelo: string;
  marca: string;
  btu: string;
...
export const buscarProdutosNovos = async (): Promise<ProdutoNovoFormatado[]> => {
  try {
    const response = await fetch(`${API_CONFIG.BACKEND_URL}/api/anuncios/novos`, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`Erro ao buscar produtos novos: ${response.status}`);
    }
    ...
    if (result.success && result.anuncios) {
      ...
      return result.anuncios.map((produto: ProdutoNovo): ProdutoNovoFormatado => {
        ...
        return {
          ...
          desconto,
          frete: 'Grátis', // Produtos novos geralmente têm frete grátis
          disponivel: true,
          loja: {
            nome: 'Loja Oficial',
            avaliacao: 4.8,
