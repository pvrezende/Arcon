...
const PropostaCliente = ({ navigation, route }: any) => {
      ...
      const propostasFormatadas: PropostaData[] = await Promise.all(
        propostasCompletas.map(async (item: DatabaseItem) => {
          ...
          const proposta = {
            ...
            descricao: item.mensagem || item.descricao,
          };
  ...
  try {
    // 1. CAPTURA OS DADOS DA PROPOSTA ANTES DE O CARD SER REMOVIDO DA LISTA
    ...
    if (response.ok) {
      ...
      // Recarrega as propostas
      await fetchPropostas();
      
      // Se foi "aceitar", cria o chat e abre
      if (acao === "aceitar" && propostaAceitaParaChat && userData) {
        try {
          console.log('🚀 Criando chat para proposta:', propostaAceitaParaChat);
          console.log('📋 solicitacao_id:', propostaAceitaParaChat.solicitacao_id);
          
          // Usar a nova rota de iniciar chat com id_solicitacao
          const chatResponse = await fetch(`${API_CONFIG.BACKEND_URL}${API_CONFIG.ENDPOINTS.CHAT.INICIAR}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({
              id_solicitacao: propostaAceitaParaChat.solicitacao_id
            })
          });

          const chatData = await chatResponse.json();
          
          if (chatResponse.ok && chatData.id_chat) {
            console.log('✅ Chat iniciado com sucesso:', chatData);
            
            // Navegar para o chat com todas as informações necessárias
            navigation.navigate("ChatScreen", { 
              chatId: chatData.id_chat.toString(),
              otherUser: {
                id: propostaAceitaParaChat.prestador_id,
                name: propostaAceitaParaChat.prestador,
                profile_image: undefined
              },
              currentUserId: userData.id_usuario || userData.id
            });
          } else {
            console.error('❌ Erro ao criar chat:', chatData);
            Alert.alert(
              'Erro ao Criar Chat', 
              'Não foi possível criar o chat. Verifique se a proposta foi aceita corretamente.'
            );
            // Não navega para o chat se não foi criado com sucesso
            return;
          }
        } catch (chatError) {
          ...
          Alert.alert(
            'Erro ao Criar Chat', 
            'Não foi possível criar o chat devido a um erro de conexão. Tente novamente.'
          );
          // Não navega para o chat se houve erro
          return;
        }
      }
    } else {
  ...
  // As propostas já vêm filtradas do backend (sem propostas aceitas/recusadas)
    ...
    content = (
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        ...
        <View style={[
          ...
          screenWidth > 633 && styles.cardsContainerDesktop,
          screenWidth <= 633 && screenWidth > 480 && styles.cardsContainerMedium,
          screenWidth <= 480 && styles.cardsContainerMobile
        ]}>
...
const styles = StyleSheet.create({
  ...
  // Desktop: 3 cards por linha (largura > 633px)
  cardsContainerDesktop: {
    paddingHorizontal: 20,
    gap: 20,
  },
  // Medium: 2 cards por linha (largura <= 633px e > 480px)
  cardsContainerMedium: {
    paddingHorizontal: 16,
    gap: 16,
  },
  // Mobile: 1 card por linha (largura <= 480px)
